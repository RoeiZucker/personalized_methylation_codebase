...
class EpochPredictionCallback(TrainerCallback):
    ...
    def _write_predictions(self, output_path, predictions):
        is_classification = predictions.ndim == 3 and predictions.shape[-1] > 1
        num_classes = int(predictions.shape[-1]) if is_classification else 0
            ...
            for example_index in range(len(self.metadata_dataset)):
                ...
                for token_index in np.flatnonzero(labels != -100):
                    ...
                    if is_classification:
                        ...
                        logit_value = float(np.asarray(prediction_values[token_index]).reshape(-1)[0])
                        probability_value = float(_sigmoid_np(logit_value))
                        row.update(
                            {
...
class TokenRegressionTrainer(ResumeCheckpointCompatMixin, Trainer):
...
# NOTE: compute metrics func was given after train_dataset but was not needed, so it was removed
# remove this note after sucessful evaluation of model

# NOTE: removed trainer type because it was unnecessery. 
# remove this note after checking the entire pipeline 
# TODO: does it need both model type and trainer type? can't trainer type allways be inferd from model_type
